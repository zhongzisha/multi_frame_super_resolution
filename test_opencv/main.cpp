...
void sum_by_indices2(float *in1, float *in2, int *indices, int size, int width, int height, int channels,
    ...
    for(int index=0;index<size;index++) {
        ...
        col = indices[index]%width;
        for(int c=0;c<channels;c++){
            out1[c] += in1[row*width*channels+col*channels+c];
            out2[c] += in2[row*width*channels+col*channels+c];
        }
    }
}
void test_defog() {
...
void test_defog_cuda2(int argc, char** argv) {
//    bool debug = true;
...
//    cv::Mat Ipar = cv::imread("F:/wy/ImageBest_tiff16.tiff", cv::IMREAD_ANYCOLOR|cv::IMREAD_ANYDEPTH);
    ...
    switch(inputType) {
    case 1:{
        Iper_orig = cv::imread("F:/wy/ImageWorst_tiff16.tiff", cv::IMREAD_ANYCOLOR|cv::IMREAD_ANYDEPTH);
        Ipar_orig = cv::imread("F:/wy/ImageBest_tiff16.tiff", cv::IMREAD_ANYCOLOR|cv::IMREAD_ANYDEPTH);
    };
        break;
    case 2:{
        cv::Mat I0 = cv::imread("F:/wy/degree0.tiff", cv::IMREAD_GRAYSCALE);
        cv::Mat I45 = cv::imread("F:/wy/degree45.tiff", cv::IMREAD_GRAYSCALE);
        cv::Mat I90 = cv::imread("F:/wy/degree90.tiff", cv::IMREAD_GRAYSCALE);
...
//    cv::Mat Iper=cv::Mat::zeros(500, 600, CV_8UC3);
//    cv::Mat Ipar=cv::Mat::zeros(500, 600, CV_8UC3);
//    int e1 = std::min(Iper_orig.cols, 600);
//    int e2 = std::min(Iper_orig.rows, 500);
//    cv::Mat tempMat1 = cv::Mat(Iper_orig, cv::Rect(0, 0, e1, e2));
//    cv::Mat tempMat2 = cv::Mat(Ipar_orig, cv::Rect(0, 0, e1, e2));
//    tempMat1.copyTo(Iper(cv::Rect(0, 0, e1, e2)));
//    tempMat2.copyTo(Ipar(cv::Rect(0, 0, e1, e2)));
//    showImg(Iper, "Iper");
//    showImg(Ipar, "Ipar");
    int num_images = 1;
    cv::TickMeter tm;
